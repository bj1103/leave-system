...
def valid_date(absence_date, absence_type):
    ...
    if absence_type == "補休" and absence_date > today + timedelta(days=1) * (over_8_pm):
        return True
    elif absence_type == "夜假" and absence_date >= today + timedelta(days=1) * (over_8_pm):
        return True
...
class AbsenceDate(State):
    ...
    def next(self, user_input, user_info):
        if "取消" in user_input:
            ...
            today = datetime.now(taipei_timezone).replace(hour=0, minute=0, second=0, microsecond=0)
...
class AbsenceConfirm(State):
    ...
    def next(self, user_input, user_info):
        if user_input == "確定":
            if valid_date(user_info["absence_date"], user_info["absence_type"]):
...
class NightTimeoff(OtherTimeoff):
    ...
    def update_nigth_timeoff_sheet(self, df, worksheet, user_info):
        ...
        worksheet.update_cell(row=target_row_id+2, col=5, value=user_info["absence_date"].strftime('%Y/%-m/%-d'))

    def generate_message(self, user_info):
        try:
            night_timeoff_sheet = gc.open_by_key(NIGHT_TIMEOFF_SHEET_KEY)
            worksheet = night_timeoff_sheet.worksheet(f"{user_info['session']}T{user_info['name']}")
            df = pd.DataFrame(worksheet.get_all_records())
...
class CancelTimeoff(State):
    ...
    def get_future_timeoff(self, df):
        ...
        for i, row in df.iterrows():
            ...
            row_date = taipei_timezone.localize(datetime(year=year, month=month, day=day))
...
class FinishCancelTimeoff(State):
    ...
    def generate_message(self, user_info):
        ...
        idx = df[(df['日期'] == date) & (df['假別'] == user_info['absence_type'])].index
        if len(idx):
        ...
        if user_info["absence_type"] == "夜假":
            ...
            if len(idx):
                idx_int = int(idx[-1])+2
                worksheet.update_cell(row=idx_int, col=5, value=f'=IF(D{idx_int}="", "", IF(D{idx_int}<TODAY(), "expired", ""))')
            else:
                fail = True

        if fail == False:
            user_message = [TextMessage(
                text=f"已幫您取消申請，請至群組確認取消資訊",
            )]
            group_message = [TextMessage(
                text=f"== 取消請假 == {user_info['absence_date'].strftime('%Y/%-m/%-d')} [{user_info['session']}梯次］{user_info['name']} {user_info['unit']} 夜假",
            )]
            return {
                "user": user_message,
                "group": group_message
            ...
            message = user_message = [TextMessage(
                text="取消失敗，請重新操作 (請注意，若超過晚上8點，則不能取消今日之夜假或明日之補休)",
            )]
            return {
                "user": message,
                "group": None
